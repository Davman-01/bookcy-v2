    ...
    <div className="min-h-screen bg-[#FAF7F2] font-['DM Sans'] pt-10 pb-20 px-4 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-[550px] relative z-10 animate-in fade-in zoom-in-95 duration-500">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-white rounded-2xl shadow-sm border border-slate-200 mb-4 text-[#E8622A]"><User size={32} /></div>
        ...
        <div className="bg-white p-1.5 rounded-2xl flex gap-1 shadow-sm border border-slate-200 mb-8">
          <button onClick={() => {setActiveTab('search'); setErrorMsg(''); setSuccessMsg('');}} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'search' ? 'bg-[#2D1B4E] text-white shadow-md' : 'bg-transparent text-slate-400 hover:text-[#2D1B4E]'}`}>Misafir Sorgulama</button>
          <button onClick={() => {setActiveTab('login'); setErrorMsg(''); setSuccessMsg('');}} className={`flex-1 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'login' ? 'bg-[#2D1B4E] text-white shadow-md' : 'bg-transparent text-slate-400 hover:text-[#2D1B4E]'}`}>Üye Girişi / Kayıt</button>
        </div>

        {errorMsg && (<div className="bg-red-50 border border-red-100 text-red-500 p-4 rounded-2xl mb-6 flex items-start gap-3 text-sm font-bold shadow-sm animate-in slide-in-from-top-2"><AlertCircle size={18} className="shrink-0 mt-0.5"/><p>{errorMsg}</p></div>)}
        {successMsg && (<div className="bg-green-50 border border-green-100 text-green-600 p-4 rounded-2xl mb-6 flex items-start gap-3 text-sm font-bold shadow-sm animate-in slide-in-from-top-2"><CheckCircle2 size={18} className="shrink-0 mt-0.5"/><p>{successMsg}</p></div>)}
                ...
                {searchResults.map((appt) => (
                  <div key={appt.id} className={`bg-white p-6 rounded-[32px] shadow-sm border relative overflow-hidden transition-all ${appt.status === 'İptal' ? 'border-red-100 opacity-70' : 'border-slate-200 hover:border-[#E8622A]'}`}>
                    ...
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 bg-orange-50 text-[#E8622A] rounded-xl flex items-center justify-center shrink-0 border border-orange-100">{appt.shops?.category === 'Bar & Club' ? <Music size={20}/> : <Scissors size={20}/>}</div>
                      <div className="flex-1">
                        ...
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 grid grid-cols-2 gap-2 mb-4">
                          <div><span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block">Tarih</span><span className="font-bold text-sm text-[#2D1B4E]">{appt.appointment_date}</span></div>
                          <div><span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block">Saat</span><span className="font-black text-sm text-[#E8622A]">{appt.appointment_time}</span></div>
                          <div className="col-span-2 pt-2 border-t border-slate-200 mt-1"><span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block">Hizmet / Uzman</span><span className="font-bold text-sm text-[#2D1B4E]">{appt.service_name} {appt.staff_name && appt.staff_name !== 'Genel' ? `• ${appt.staff_name}` : ''}</span></div>
                        </div>
                        {(!appt.status || appt.status === 'Bekliyor') && (<button onClick={() => handleCancelAppointment(appt)} className="w-full bg-white border border-red-200 text-red-500 hover:bg-red-50 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-colors cursor-pointer flex items-center justify-center gap-2"><XCircle size={16}/> İşlemi İptal Et</button>)}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {activeTab === 'login' && (
          <div className="bg-white p-8 rounded-[32px] shadow-xl border border-slate-200 animate-in slide-in-from-right-8 duration-300">
            <h3 className="font-black text-[#2D1B4E] text-lg mb-6 flex items-center gap-2"><LogIn className="text-[#E8622A]"/> Sisteme Giriş Yapın</h3>
            <div className="space-y-3 mb-6">
              <button onClick={() => handleSocialLogin('google')} className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 py-4 rounded-2xl font-bold text-sm hover:bg-slate-50 transition-all hover:border-[#E8622A] cursor-pointer shadow-sm text-[#2D1B4E]"><GoogleIcon /> Google ile Devam Et</button>
              <button onClick={() => handleSocialLogin('apple')} className="w-full flex items-center justify-center gap-3 bg-[#2D1B4E] text-white py-4 rounded-2xl font-bold text-sm hover:bg-[#1a0f2e] transition-all cursor-pointer shadow-md"><AppleIcon /> Apple ile Devam Et</button>
            </div>
            <div className="relative mb-6 text-center"><hr className="border-slate-100" /><span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-4 text-[10px] font-black text-slate-300 uppercase tracking-widest">Veya E-Posta İle</span></div>
            <form onSubmit={handleEmailLogin} className="space-y-4">
              <div className="relative"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Mail size={18}/></div><input type="email" required value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="E-posta Adresiniz" className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 pl-12 pr-4 font-bold text-sm outline-none focus:border-[#E8622A] transition-colors" /></div>
              <div className="relative"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Lock size={18}/></div><input type="password" required value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="Şifreniz" className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 pl-12 pr-4 font-bold text-sm outline-none focus:border-[#E8622A] transition-colors" /></div>
              <button type="submit" disabled={loading} className="w-full bg-[#E8622A] text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-lg hover:bg-orange-600 transition-all border-none cursor-pointer">{loading ? <Loader2 className="animate-spin mx-auto" size={18}/> : 'Giriş Yap'}</button>
            </form>
            <p className="text-center mt-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">Hesabınız yoksa Google veya Apple ile giriş yaptığınızda <span className="text-[#E8622A]">otomatik olarak</span> hesabınız oluşturulur.</p>
          </div>
        )}
      </div>
    </div>
