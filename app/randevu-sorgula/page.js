...
export default function CustomerPortal() {
  ...
  const handleCancelAppointment = async (appointment) => {
    ...
    try {
      ...
      // Bekleme Listesi API'mizi tetikle
      try {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: provider,
        options: {
          redirectTo: `${window.location.origin}/profilim`, 
        }
      });
      if (error) throw error;
    } catch (err) {
  ...
  // 4. KLASİK E-POSTA GİRİŞİ (Hazırlık)
      ...
      <div className="w-full max-w-[550px] relative z-10 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="bg-white p-1.5 rounded-2xl flex gap-1 shadow-sm border border-slate-200 mb-8">
          ...
          <button 
            ...
            className={`flex-1 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'login' ? 'bg-[#2D1B4E] text-white shadow-md' : 'bg-transparent text-slate-400 hover:text-[#2D1B4E]'}`}
          >
            Üye Girişi / Kayıt
          </button>
        </div>
