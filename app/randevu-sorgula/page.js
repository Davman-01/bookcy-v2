...
import { Search, CalendarX, CheckCircle2, Clock, Phone, AlertCircle } from 'lucide-react';
...
export default function AppointmentCheckPage() {
  ...
  const cancelAppointment = async (appt) => {
    ...
    if (!error) {
      // İŞLETMEYE MAİL AT (Opsiyonel API çağrısı)
    ...
    <div className="min-h-screen bg-[#F8FAFC] pt-32 pb-20 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 mb-12 max-w-md mx-auto">
          ...
          <button className="bg-[#2D1B4E] text-white px-8 rounded-2xl font-black text-xs uppercase cursor-pointer hover:bg-[#E8622A] transition-all">SORGULA</button>
        </form>
            ...
            appointments.map(appt => (
              <div key={appt.id} className="bg-white p-6 rounded-[24px] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4 text-left w-full">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${appt.status === 'cancelled' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'}`}>
                ...
                {appt.status !== 'cancelled' && (
                  <button 
                    ...
                    className="w-full md:w-auto bg-red-50 text-red-600 px-6 py-3 rounded-xl font-black text-[10px] uppercase border border-red-100 hover:bg-red-600 hover:text-white transition-all cursor-pointer"
                  >
                    RANDEVUYU İPTAL ET
                  </button>
                )}
