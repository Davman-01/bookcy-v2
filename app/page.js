...
export default function Home() {
  ...
  return (
    <div className="w-full">
      ...
      {/* HERO BÖLÜMÜ - "justify-between" ile üst-orta-alt dengesi kuruldu. Gereksiz paddingler kısıldı. */}
      <section className="relative w-full min-h-[100dvh] flex flex-col items-center justify-between pt-[90px] md:pt-[100px] pb-6 md:pb-10 bg-[#1a0f2e] overflow-hidden">
        ...
        {/* ANA İÇERİK - "flex-1" ile dikeyde tam merkeze dinamik olarak oturur */}
        <div className="relative z-20 flex flex-col items-center justify-center flex-1 w-full max-w-[850px] mx-auto px-4 text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-1.5 text-[10px] md:text-xs font-bold text-white/90 uppercase tracking-widest mb-4 md:mb-5 backdrop-blur-md shadow-lg">
            <div className="w-2 h-2 rounded-full bg-[#E8622A] animate-pulse"></div>
            {text.hero?.eyebrow}
          </div>
          
          <h1 className="text-3xl md:text-5xl font-black text-white tracking-tight mb-4 md:mb-5 leading-[1.1] drop-shadow-xl">
          ...
          <p className="text-sm md:text-base lg:text-lg text-white/80 mb-6 md:mb-8 max-w-xl mx-auto font-medium drop-shadow-md leading-relaxed px-2">
            {text.hero?.sub}
          </p>
          ...
          <div className="flex items-center justify-center flex-wrap gap-2 text-white/80 text-[10px] md:text-xs font-bold uppercase tracking-widest hidden md:flex">
             ...
             {(categories || []).slice(0,4).map(c => (
               <button key={c.key} type="button" onClick={() => router.push(`/isletmeler?c=${c.dbName}`)} className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 backdrop-blur-sm px-4 py-2 rounded-full border border-white/20 transition-all cursor-pointer text-white font-bold hover:-translate-y-0.5">
        ...
        {/* İSTATİSTİKLER - Ekranın en altına yapışır ama taşmaz */}
        <div className="w-full relative z-20 px-4 mt-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 border-t border-white/10 pt-4 md:pt-6 max-w-[850px] mx-auto backdrop-blur-sm bg-white/5 rounded-[32px] pb-4 md:pb-6 px-4 md:px-8">
