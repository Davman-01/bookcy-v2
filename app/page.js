...
import { 
  ...
  Smartphone, Grid, X, Gem, Check, PieChart, Store, CalendarOff, Music, Ticket, HeartHandshake, Sun, Moon, ShieldCheck
} from 'lucide-react';
...
export default function Home() {
  ...
  const t = {
    ...
    EN: {
      ...
      footer: { desc: "North Cyprus's premier booking platform for beauty & wellness.", links: "Platform", cities: "Regions", legal: "Terms", terms: "Terms of Use", privacy: "Privacy Policy", copy: "All rights reserved. Made in North Cyprus. 🇹🇷", kvkk: "KVKK", cookies: "Cookie Policy" },
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        body { background: var(--c-bg-main); color: var(--c-text-main); font-family: 'DM Sans', sans-serif; overflow-x: hidden; transition: background-color 0.3s, color 0.3s; }
        ...
        .lang-pills { display: flex; flex-direction: row; gap: 4px; } .lang-pill { font-size: 11px; font-weight:600; padding: 4px 10px; border-radius: 20px; border: 1px solid transparent; transition: all 0.2s; color: var(--c-text-muted); cursor:pointer;} .lang-pill.active { background: var(--fig); color: white; border-color: var(--fig); } .lang-pill:hover:not(.active) { border-color: var(--c-border); color: var(--c-text-main); }
        ...
        .cat-card { display:flex; flex-direction:column; align-items:center; gap:12px; transition:transform 0.25s; cursor:pointer;} .cat-card:hover { transform:translateY(-6px); }
        ...
        .venue-card { border-radius:24px; overflow:hidden; background: var(--c-bg-sub); transition:transform 0.3s, box-shadow 0.3s; position:relative; display:flex; flex-direction:column; cursor:pointer;} .venue-card:hover { transform:translateY(-8px); box-shadow:0 24px 60px rgba(0,0,0,0.15); } .venue-card.featured { grid-row: span 2; }
        ...
        .section-cta { padding:80px 48px; } .cta-inner { max-width:1200px; margin:0 auto; background:var(--fig); border-radius:32px; padding:72px 80px; display:flex; align-items:center; gap:64px; position:relative; overflow:hidden; }
      ...
      <nav>
        <div className="nav-logo" onClick={() => {setStep('services'); setShowLogin(false); setShowRegister(false); window.scrollTo(0,0);}}>
          <svg className="nav-logo-icon" viewBox="0 0 36 36" fill="none" aria-hidden="true"><rect width="36" height="36" rx="10" fill="#2D1B4E"/><circle cx="18" cy="10" r="4.5" fill="#F5C5A3"/><rect x="10" y="15.5" width="4" height="15" rx="2" fill="#F5C5A3"/><path d="M14 15.5 Q23 15.5 23 20 Q23 24.5 14 24.5" stroke="#F5C5A3" strokeWidth="3.5" fill="none" strokeLinecap="round"/><path d="M14 24.5 Q25 24.5 25 29 Q25 33.5 14 33.5" stroke="#E8622A" strokeWidth="3.5" fill="none" strokeLinecap="round"/><circle cx="28" cy="8" r="4.5" fill="#E8622A"/><polyline points="25.5,8 27.5,10 31,6" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/></svg>
                      ...
                      <div className="absolute top-[68px] left-1/2 -translate-x-1/2 w-screen bg-[#1b0f30] text-white shadow-2xl border-t border-slate-800 cursor-default animate-in slide-in-from-top-2 duration-200">
                          <div className="max-w-[1000px] mx-auto py-12 px-8">
                              <div className="grid grid-cols-4 gap-8 mb-10 text-left">
                                  <div><h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col1Title}</h4><ul className="space-y-5 font-bold text-slate-300 capitalize text-sm">{['profile', 'market', 'team'].map(key => <li key={key}><button onClick={() => goToFeature(key)} className="hover:text-white cursor-pointer transition-colors flex items-center gap-2 text-decoration-none bg-transparent border-none text-slate-300 p-0"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</button></li>)}</ul></div>
                                  <div><h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col2Title}</h4><ul className="space-y-5 font-bold text-slate-300 capitalize text-sm">{['booking', 'app'].map(key => <li key={key}><button onClick={() => goToFeature(key)} className="hover:text-white cursor-pointer transition-colors flex items-center gap-2 text-decoration-none bg-transparent border-none text-slate-300 p-0"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</button></li>)}</ul></div>
                                  <div><h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col3Title}</h4><ul className="space-y-5 font-bold text-slate-300 capitalize text-sm">{['marketing', 'calendar', 'crm'].map(key => <li key={key}><button onClick={() => goToFeature(key)} className="hover:text-white cursor-pointer transition-colors flex items-center gap-2 text-decoration-none bg-transparent border-none text-slate-300 p-0"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</button></li>)}</ul></div>
                                  <div><h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col4Title}</h4><ul className="space-y-5 font-bold text-slate-300 capitalize text-sm">{['boost', 'stats'].map(key => <li key={key}><button onClick={() => goToFeature(key)} className="hover:text-white cursor-pointer transition-colors flex items-center gap-2 text-decoration-none bg-transparent border-none text-slate-300 p-0"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</button></li>)}</ul></div>
                              </div>
                              <div className="flex justify-center border-t border-slate-800 pt-8"><button onClick={() => {setStep('all_features'); setShowFeaturesMenu(false); window.scrollTo(0,0);}} className="border border-slate-700 bg-[#1b0f30] text-white px-8 py-3 rounded-lg font-bold hover:bg-slate-800 hover:text-[#E8622A] transition-colors text-decoration-none cursor-pointer">{t[lang].megaMenu.btn}</button></div>
                          </div>
                      </div>
                  )}
              </div>
        ...
        <div className="nav-right">
          <div className="lang-pills hidden md:flex"><button onClick={()=>setLang('TR')} className={`lang-pill ${lang==='TR' ? 'active' : ''}`}>TR</button><button onClick={()=>setLang('EN')} className={`lang-pill ${lang==='EN' ? 'active' : ''}`}>EN</button><button onClick={()=>setLang('RU')} className={`lang-pill ${lang==='RU' ? 'active' : ''}`}>RU</button></div>
          <button onClick={toggleTheme} aria-label="Tema Değiştir" className="p-2 rounded-full border border-[var(--c-border)] text-[var(--c-text-main)] hover:bg-[var(--c-bg-sub)] transition-colors cursor-pointer mr-2 bg-transparent">{theme === 'dark' ? <Sun size={16}/> : <Moon size={16}/>}</button>
          {loggedInShop ? ( <div className="flex gap-2 items-center"><button onClick={handleLogout} className="btn-outline">{t[lang].nav.logout}</button><button onClick={() => router.push('/dashboard')} className="btn-primary"><UserCircle size={18}/> <span>{t[lang].nav.dashboard}</span></button></div> ) : ( <><button onClick={() => setShowRegister(true)} className="btn-outline">{t[lang].nav.addShop}</button><button onClick={() => setShowLogin(true)} className="btn-primary"><UserCircle size={18}/><span>{t[lang].nav.login}</span></button></> )}
        </div>
      </nav>
      ...
      {showRegister && (
          <div className="fixed inset-0 w-screen h-screen bg-[#2D1B4E]/80 backdrop-blur-md z-[9999] flex items-center justify-center p-4 overflow-y-auto pt-20">
            <div className="bg-white border border-slate-200 w-full max-w-[800px] rounded-[32px] p-8 md:p-10 relative shadow-2xl my-auto animate-in zoom-in-95 duration-300">
              ...
              {registerSuccess ? (
                  <div className="text-center py-20"><CheckCircle2 size={64} className="mx-auto text-[#00c48c] mb-6" /><h2 className="text-2xl md:text-3xl font-black text-[#E8622A] uppercase italic mb-4">{t[lang].reg.success}</h2><div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 inline-block text-left mt-4 text-[#2D1B4E] w-full max-w-sm"><p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-4">Banka Bilgileri / Bank Details:</p><p className="font-bold text-sm">Banka: <span className="font-normal">İş Bankası</span></p><p className="font-bold text-sm">Alıcı: <span className="font-normal">BOOKCY LTD.</span></p><p className="font-bold text-sm">IBAN: <span className="text-[#E8622A]">TR99 0006 4000 0012 3456 7890 12</span></p><p className="text-sm font-bold text-slate-500 mt-6 mb-4 text-center">Bizi tercih ettiğiniz için teşekkür ederiz. İşletme profilinizin onaylanıp yayına alınabilmesi için lütfen ödeme dekontunuzu WhatsApp destek hattımız üzerinden bize iletiniz.</p><a href="https://wa.me/905555555555?text=Merhaba,%20Bookcy%20işletme%20kaydımı%20yaptım.%20Dekontumu%20iletiyorum." target="_blank" rel="noreferrer" className="w-full bg-[#25D366] text-white font-black py-4 rounded-xl uppercase tracking-widest hover:bg-green-600 transition-colors shadow-lg shadow-green-500/30 flex justify-center items-center gap-2 text-decoration-none text-xs border-none cursor-pointer mt-4"><MessageCircle size={18}/> DEKONTU WHATSAPP'TAN İLET</a></div></div>
              ) : (
                  <><div className="flex flex-col mb-8 text-center"><h2 className="text-2xl md:text-3xl font-black uppercase tracking-tight italic text-[#2D1B4E]">{t[lang].reg.title}</h2><p className="text-[10px] text-[#E8622A] font-bold uppercase tracking-[0.2em] mt-2 flex items-center justify-center gap-2"><Lock size={12}/> {t[lang].reg.subtitle}</p></div><form onSubmit={handleRegisterSubmit} className="flex flex-col gap-5"><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input required placeholder={t[lang].reg.shopName} className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.name} onChange={e => setNewShop({...newShop, name: e.target.value})} /><select className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold text-[#2D1B4E] outline-none focus:border-[#E8622A] cursor-pointer" value={newShop.category} onChange={e => setNewShop({...newShop, category: e.target.value})}>{categories.map(c => <option key={c.dbName} value={c.dbName}>{t[lang].cats[c.key]}</option>)}</select></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><select required className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold text-[#2D1B4E] outline-none focus:border-[#E8622A] cursor-pointer uppercase" value={newShop.location} onChange={e => setNewShop({...newShop, location: e.target.value})}>{cyprusRegions.map(region => <option key={region} value={region}>{region}</option>)}</select><input required placeholder={t[lang].reg.address} className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none uppercase text-[#2D1B4E]" value={newShop.address} onChange={e => setNewShop({...newShop, address: e.target.value})} /></div><div className="border-t border-slate-100 pt-5"><input type="url" placeholder={t[lang].reg.maps} className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.maps_link} onChange={e => setNewShop({...newShop, maps_link: e.target.value})} /></div><div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-slate-100 pt-5"><div className="flex gap-2 w-full relative"><select className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-3 outline-none focus:border-[#E8622A] font-bold text-xs text-[#2D1B4E] cursor-pointer w-20 shrink-0" value={newShop.phoneCode} onChange={e => setNewShop({...newShop, phoneCode: e.target.value})}><option value="+90">TR</option><option value="+357">CY</option><option value="+44">UK</option></select><div className="relative flex-1"><input required type="tel" placeholder={t[lang].reg.contactPhone} className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 px-4 pr-10 outline-none focus:border-[#E8622A] font-bold text-xs text-[#2D1B4E]" value={newShop.contactPhone} onChange={handlePhoneChange} />{phoneValid === true && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500" size={16}/>}{phoneValid === false && <XCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500" size={16}/>}</div></div><input placeholder={t[lang].reg.contactInsta} className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.contactInsta} onChange={e => setNewShop({...newShop, contactInsta: e.target.value})} /><div className="relative"><input type="email" placeholder={t[lang].reg.contactEmail} className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 pr-10 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.contactEmail} onChange={handleAdminEmailChange} />{adminEmailValid === true && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500" size={16}/>}{adminEmailValid === false && <XCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500" size={16}/>}</div></div><div className="border-t border-slate-100 pt-5 relative"><input required type="email" placeholder={t[lang].reg.email} className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 pr-10 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.email} onChange={handleEmailChange} />{emailValid === true && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500" size={16}/>}{emailValid === false && <XCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500" size={16}/>}</div><textarea placeholder={t[lang].reg.desc} rows="2" className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none resize-none text-[#2D1B4E]" value={newShop.description} onChange={e => setNewShop({...newShop, description: e.target.value})}></textarea><div className="bg-slate-50 border border-slate-200 border-dashed rounded-2xl p-4 relative group hover:border-[#E8622A] transition-all">{newShop.logoFile ? <span className="text-[10px] font-bold text-[#00c48c] flex items-center justify-center gap-2"><CheckCircle2 size={16}/> {newShop.logoFile.name}</span> : <div className="flex flex-col items-center justify-center text-center cursor-pointer"><Upload size={20} className="text-slate-400 mb-2 group-hover:text-[#E8622A]" /><span className="text-[10px] font-bold text-slate-500 uppercase">{t[lang].reg.upload}</span></div>}<input type="file" accept=".png, .jpg, .jpeg" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={e => setNewShop({...newShop, logoFile: e.target.files[0]})} /></div><div className="border-t border-slate-100 pt-4"><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-3">{t[lang].reg.pack}</p><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{packages.map(p => (<div key={p.name} onClick={() => setNewShop({...newShop, package: p.name})} className={`cursor-pointer p-5 rounded-2xl border transition-all ${newShop.package === p.name ? 'bg-orange-50 border-[#E8622A]' : 'bg-white border-slate-200 hover:border-[#E8622A]'}`}><div className="flex justify-between items-center mb-2"><h4 className={`text-sm font-black uppercase ${newShop.package === p.name ? 'text-[#E8622A]' : 'text-[#2D1B4E]'}`}>{p.name}</h4>{newShop.package === p.name && <CheckCircle2 size={16} className="text-[#E8622A]"/>}</div><p className="text-xs font-bold text-slate-500">{p.price}</p></div>))}</div></div><div className="grid grid-cols-2 gap-4 mt-2 border-t border-slate-100 pt-4"><input required placeholder={t[lang].reg.user} className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.username} onChange={e => setNewShop({...newShop, username: e.target.value})} /><input required placeholder={t[lang].reg.pass} className="bg-slate-50 border border-slate-200 rounded-2xl py-4 px-6 text-xs font-bold focus:border-[#E8622A] outline-none text-[#2D1B4E]" value={newShop.password} onChange={e => setNewShop({...newShop, password: e.target.value})} /></div><button type="submit" disabled={isUploading || emailValid === false || phoneValid === false || adminEmailValid === false} className="w-full btn-primary justify-center py-5 rounded-2xl mt-2 uppercase text-xs tracking-[0.2em] shadow-lg border-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">{isUploading ? t[lang].reg.uploading : t[lang].reg.submit}</button></form></>
              )}
            </div>
          </div>
      )}
      ...
      {showLogin && (
        <div className="fixed inset-0 bg-[#2D1B4E]/80 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-[480px] rounded-[32px] p-8 md:p-12 relative animate-in zoom-in-95 duration-300 shadow-2xl border border-slate-200">
            <button onClick={() => setShowLogin(false)} className="absolute top-6 right-6 text-slate-400 hover:text-[#E8622A] bg-transparent border-none cursor-pointer"><X size={28}/></button>
            <div className="text-center mb-8"><div className="w-16 h-16 bg-[#E8622A]/10 text-[#E8622A] rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-inner"><Store size={32} /></div><h1 className="text-2xl md:text-3xl font-black text-[#2D1B4E] uppercase tracking-tight mb-2">GİRİŞ YAP</h1><p className="text-slate-400 font-bold text-xs uppercase tracking-widest">BOOKCY İŞ ORTAĞI PANELİ</p></div>
            <div className="flex bg-slate-50 p-1 rounded-xl mb-8 border border-slate-100"><button onClick={() => setLoginType('owner')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-lg transition-all border-none cursor-pointer flex justify-center items-center gap-2 ${loginType === 'owner' ? 'bg-white text-[#E8622A] shadow-sm' : 'bg-transparent text-slate-400 hover:text-slate-600'}`}><User size={14}/> YÖNETİCİ</button><button onClick={() => setLoginType('staff')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-lg transition-all border-none cursor-pointer flex justify-center items-center gap-2 ${loginType === 'staff' ? 'bg-white text-[#E8622A] shadow-sm' : 'bg-transparent text-slate-400 hover:text-slate-600'}`}><Users size={14}/> PERSONEL</button></div>
      ...
      <main className="flex-1 w-full relative z-10" style={{minHeight: '80vh'}}>
            ...
            <div className="w-full animate-in fade-in">
                <section className="hero">
                  ...
                  <div className="hero-content">
                    ...
                    <h1 className="hero-title">{t[lang].home.title1}<br/><span className="accent">{t[lang].home.title2}</span><br/><span className="accent-2">{t[lang].home.title3}</span> {t[lang].home.title4}</h1>
                    <p className="hero-sub">{t[lang].home.subtitle}</p>
                    <form className="search-wrap" onSubmit={handleHeroSearch}>
                      <div className="search-field"><span className="search-icon"><Search size={18}/></span><input aria-label="Hizmet veya Mekan Arama" type="text" placeholder={t[lang].home.searchPlace} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/></div>
                      <div className="search-divider"></div>
                      <div className="search-location"><MapPin size={16} className="text-slate-400"/><select aria-label="Bölge Seçimi" value={filterRegion} onChange={(e) => setFilterRegion(e.target.value)}><option value="All">{t[lang].home.searchLoc}</option>{cyprusRegions.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
                  ...
                  {(() => {
                      ...
                      return (
                          <div className="hero-stats">
                              <div className="stat"><div className="stat-num">{liveShopsCount.toLocaleString('tr-TR')}</div><div className="stat-label">{t[lang].home.stats.s1}</div></div>
                              <div className="stat"><div className="stat-num">{liveCustomersCount.toLocaleString('tr-TR')}</div><div className="stat-label">{t[lang].home.stats.s2}</div></div>
                              <div className="stat"><div className="stat-num">{liveApptsCount.toLocaleString('tr-TR')}</div><div className="stat-label">{t[lang].home.stats.s3}</div></div>
                              <div className="stat"><div className="stat-num">%98</div><div className="stat-label">{t[lang].home.stats.s4}</div></div>
                          </div>
                      );
                  })()}
                  <svg className="wave" viewBox="0 0 1440 80" preserveAspectRatio="none" fill="var(--c-bg-main)"><path d="M0,40 C240,80 480,0 720,40 C960,80 1200,0 1440,40 L1440,80 L0,80 Z"/></svg>
                </section>

                <section className="section-categories">
                  <div className="section-header reveal"><div><div className="section-label-sm">{t[lang].cats.catTitle}</div><div className="section-title">{t[lang].cats.catSub}</div></div><button className="see-all" onClick={()=>{setFilterService('All'); setStep('all_shops'); window.scrollTo(0,0);}}>{t[lang].cats.seeAll}</button></div>
                  <div className="categories-grid">{categories.map((c, i) => (<div key={c.key} onClick={() => { setFilterService(c.dbName); setStep('all_shops'); window.scrollTo(0,0); }} className={`cat-card reveal reveal-delay-${(i%4)+1}`}><div className="cat-img-wrap"><div className="cat-emoji-bg" style={{background: c.bg}}>{c.emoji}</div></div><div className="cat-name">{t[lang].cats[c.key]}</div></div>))}</div>
                </section>

               {recommendedShops.length > 0 && (
                  <section className="section-featured">
                    <div className="section-header"><div><div className="section-label-sm">{t[lang].homeInfo.recLabel}</div><div className="section-title">{t[lang].homeInfo.recTitle}</div></div><button className="see-all" onClick={()=>{setFilterSort('High'); setStep('all_shops'); window.scrollTo(0,0);}}>{t[lang].cats.seeAll}</button></div>
                    <div className="featured-grid">
                      {recommendedShops.map((shop, idx) => (<div key={shop.id} onClick={() => { setSelectedShop(shop); setStep('shop_profile'); setProfileTab(shop.category === 'Bar & Club' ? 'events' : 'services'); setBookingPhase(1); window.scrollTo(0,0); }} className={`venue-card ${idx === 0 ? 'featured' : ''}`}><div className="venue-img" style={{background: 'var(--c-bg-sub)'}}>{shop.cover_url || shop.logo_url ? <img alt={`${shop.name} Fotoğrafı`} loading="lazy" decoding="async" src={shop.cover_url || shop.logo_url} /> : categories.find(c=>c.dbName===shop.category)?.emoji}{idx === 0 && <div className="venue-badge hot">🔥 Çok Popüler</div>}{idx === 1 && <div className="venue-badge new">✨ Yeni</div>}<div className="venue-fav"><HeartHandshake size={14}/></div></div><div className="venue-info"><div className="venue-cat">{t[lang].cats[categories.find(c => c.dbName === shop.category)?.key || 'barber']}</div><div className="venue-name">{shop.name}</div><div className="venue-meta"><span>📍 {shop.location}</span></div><button className="venue-book-btn">{t[lang].book.btnBook}</button></div></div>))}
                    </div>
                  </section>
               )}
              ...
              <section className="section-how">
                <div className="how-inner">
                  <div className="how-header"><div className="section-label-sm" style={{color:'var(--terra)', marginBottom:'8px', fontWeight:'800', letterSpacing:'4px'}}>{t[lang].homeInfo.howLabel}</div><div className="section-title" style={{color:'white'}}>{t[lang].homeInfo.howTitle}</div></div>
                  <div className="steps-grid"><div className="step"><div className="step-icon" style={{background:'rgba(255,255,255,0.05)'}}>🔍<div className="step-num">1</div></div><div className="step-title">{t[lang].homeInfo.how1Title}</div><div className="step-desc">{t[lang].homeInfo.how1Desc}</div></div><div className="step"><div className="step-icon" style={{background:'rgba(255,255,255,0.05)'}}>📅<div className="step-num">2</div></div><div className="step-title">{t[lang].homeInfo.how2Title}</div><div className="step-desc">{t[lang].homeInfo.how2Desc}</div></div><div className="step"><div className="step-icon" style={{background:'rgba(255,255,255,0.05)'}}>✅<div className="step-num">3</div></div><div className="step-title">{t[lang].homeInfo.how3Title}</div><div className="step-desc">{t[lang].homeInfo.how3Desc}</div></div><div className="step"><div className="step-icon" style={{background:'rgba(255,255,255,0.05)'}}>✨<div className="step-num">4</div></div><div className="step-title">{t[lang].homeInfo.how4Title}</div><div className="step-desc">{t[lang].homeInfo.how4Desc}</div></div></div>
                </div>
              </section>

              <section className="section-cta">
                <div className="cta-inner">
                  <div className="cta-text"><div className="section-label-sm">{t[lang].homeInfo.ctaLabel}</div><div className="cta-title">{t[lang].homeInfo.ctaTitle1}<br/><span>{t[lang].homeInfo.ctaTitle2}</span></div><div className="cta-sub">{t[lang].homeInfo.ctaSub}</div></div>
                  <div className="cta-actions"><button onClick={()=>{setShowRegister(true); window.scrollTo(0,0);}} className="app-badge"><div className="app-badge-icon">💼</div><div className="app-badge-text"><div className="small">Hemen Katıl</div><div className="big">İşletme Ekle</div></div></button><button onClick={()=>{setStep('about'); window.scrollTo(0,0);}} className="app-badge"><div className="app-badge-icon">🚀</div><div className="app-badge-text"><div className="small">İncele</div><div className="big">Özellikler & Paketler</div></div></button></div>
                </div>
              </section>
            </div>
        )}
        ...
        {step === 'all_shops' && (
            <div className="w-full max-w-[1400px] mx-auto pt-24 px-4 md:px-8 animate-in fade-in duration-500 pb-20">
                ...
                <div className="flex justify-between items-end mb-8 border-b theme-border-sub pb-4"><h2 className="text-3xl md:text-4xl font-black uppercase theme-text-main">{t[lang].filters.title}</h2><span className="text-sm font-bold theme-text-muted">{sortedShops.length} {t[lang].filters.count}</span></div>
                <div className="flex flex-col lg:flex-row gap-8 items-start">
                    <aside className="w-full lg:w-[320px] theme-bg-card theme-border rounded-[24px] p-6 lg:sticky top-28 shrink-0 shadow-sm flex flex-col gap-8">
                        <div className="flex justify-between items-center border-b theme-border-sub pb-4"><h3 className="font-black theme-text-main uppercase tracking-widest text-xs flex items-center gap-2"><Settings size={16}/> Filtreler</h3><button onClick={() => {setFilterRegion('All'); setFilterService('All'); setSearchQuery(''); setFilterSort('High');}} className="text-[10px] font-bold text-slate-400 hover:text-[#E8622A] transition-colors uppercase bg-transparent border-none outline-none cursor-pointer">{t[lang].filters.clear}</button></div>
                        <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mb-3">{t[lang].filters.search}</p><div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} aria-hidden="true"/><input aria-label="Arama Kutusu" type="text" placeholder={t[lang].filters.search} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full theme-bg-sub theme-border-sub rounded-xl py-3 pl-10 pr-4 font-bold text-xs theme-text-main outline-none focus:border-[#E8622A] transition-colors" /></div></div>
                        <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mb-3">{t[lang].filters.region}</p><div className="flex flex-col gap-2"><label className="flex items-center gap-3 cursor-pointer group"><div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${filterRegion === 'All' ? 'bg-[#E8622A] border-[#E8622A]' : 'theme-bg-card theme-border hover:border-[#E8622A]'}`}>{filterRegion === 'All' && <Check size={14} className="text-white"/>}</div><span className={`text-sm font-bold transition-colors ${filterRegion === 'All' ? 'theme-text-main' : 'theme-text-muted group-hover:text-[#E8622A]'}`}>Tüm Bölgeler</span><input type="radio" className="hidden" checked={filterRegion === 'All'} onChange={() => setFilterRegion('All')} /></label>{cyprusRegions.map(r => (<label key={r} className="flex items-center gap-3 cursor-pointer group"><div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${filterRegion === r ? 'bg-[#E8622A] border-[#E8622A]' : 'theme-bg-card theme-border hover:border-[#E8622A]'}`}>{filterRegion === r && <Check size={14} className="text-white"/>}</div><span className={`text-sm font-bold transition-colors ${filterRegion === r ? 'theme-text-main' : 'theme-text-muted group-hover:text-[#E8622A]'}`}>{r}</span><input type="radio" className="hidden" checked={filterRegion === r} onChange={() => setFilterRegion(r)} /></label>))}</div></div>
                        <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mb-3">{t[lang].filters.service}</p><div className="flex flex-wrap gap-2"><button onClick={() => setFilterService('All')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border outline-none cursor-pointer ${filterService === 'All' ? 'bg-[var(--fig)] text-white border-[var(--fig)]' : 'theme-bg-sub theme-text-muted theme-border hover:border-[#E8622A] hover:text-[#E8622A]'}`}>Tümü</button>{categories.map(c => (<button key={c.dbName} onClick={() => setFilterService(c.dbName)} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border outline-none cursor-pointer ${filterService === c.dbName ? 'bg-[var(--fig)] text-white border-[var(--fig)]' : 'theme-bg-sub theme-text-muted theme-border hover:border-[#E8622A] hover:text-[#E8622A]'}`}>{t[lang].cats[c.key]}</button>))}</div></div>
                        <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mb-3">Sıralama</p><div className="relative theme-bg-sub rounded-xl theme-border-sub border"><select aria-label="Sıralama Seçenekleri" value={filterSort} onChange={(e) => setFilterSort(e.target.value)} className="w-full bg-transparent border-none py-3 pl-4 pr-10 font-bold text-xs theme-text-main outline-none appearance-none cursor-pointer"><option value="High">{t[lang].filters.sortHigh}</option><option value="Low">{t[lang].filters.sortLow}</option></select><SlidersHorizontal size={14} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"/></div></div>
                    </aside>
                    <div className="flex-1 w-full flex flex-col gap-5">
                        {sortedShops.map((shop) => (
                            <div key={shop.id} onClick={() => { setSelectedShop(shop); setStep('shop_profile'); setProfileTab(shop.category === 'Bar & Club' ? 'events' : 'services'); setBookingPhase(1); window.scrollTo(0,0); }} className="venue-card flex flex-col md:flex-row items-center justify-between p-5 md:p-6 theme-bg-card theme-border hover:border-[#E8622A] transition-colors rounded-[24px] cursor-pointer">
                                <div className="flex items-center gap-6 w-full"><div className="w-20 h-20 md:w-28 md:h-28 rounded-full theme-bg-sub flex items-center justify-center font-black text-[#E8622A] text-xs shadow-inner overflow-hidden theme-border-sub border shrink-0 relative">{(shop.package === 'Premium' || shop.package === 'Premium Paket') && (<div className="absolute inset-0 border-4 border-yellow-400 rounded-full shadow-[0_0_15px_rgba(250,204,21,0.5)]"></div>)}{shop.logo_url ? <img alt={`${shop.name} Logosu`} loading="lazy" decoding="async" src={shop.logo_url} className="w-full h-full object-cover" /> : "LOGO"}</div><div className="flex-1"><div className="flex items-center gap-3 mb-1"><h3 className="text-xl md:text-2xl font-black uppercase theme-text-main transition-colors">{shop.name}</h3>{(shop.package === 'Premium' || shop.package === 'Premium Paket') && <Gem size={16} className="text-yellow-500 fill-yellow-500 drop-shadow-sm"/>}</div><div className="flex flex-wrap items-center gap-3 mt-2 theme-text-muted text-[10px] md:text-xs font-bold uppercase tracking-widest"><span className="flex items-center theme-text-main theme-bg-sub px-2 py-1 rounded-md"><Briefcase size={12} className="mr-1"/> {t[lang].cats[categories.find(c => c.dbName === shop.category)?.key || 'barber']}</span><span className="flex items-center"><MapPin size={12} className="mr-1"/> {shop.address || shop.location}</span></div></div></div>
                                <button className="hidden md:flex btn-primary mt-4 md:mt-0 shrink-0 border-none outline-none">{t[lang].book.btnBook}</button>
                            </div>
                        ))}
                        {sortedShops.length === 0 && (<div className="text-center py-32 theme-bg-card rounded-[32px] theme-border border"><Search size={48} className="mx-auto text-slate-400 mb-4"/><p className="theme-text-muted font-bold uppercase tracking-widest">{t[lang].shops.empty}</p></div>)}
                    </div>
                </div>
            </div>
        )}
        ...
        {step === 'shop_profile' && selectedShop && (
            <div className="w-full max-w-6xl mx-auto pt-24 px-4 animate-in fade-in duration-500 pb-20">
                ...
                <div className="w-full h-[250px] md:h-[350px] rounded-[32px] overflow-hidden relative mb-16 theme-border border theme-bg-sub shadow-sm">{selectedShop.cover_url ? <img alt={`${selectedShop.name} Kapak Görseli`} loading="lazy" decoding="async" src={selectedShop.cover_url} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gradient-to-r from-[var(--c-border-sub)] to-[var(--c-bg-sub)]"></div>}<div className="absolute -bottom-10 left-8 flex items-end gap-6"><div className="w-24 h-24 md:w-32 md:h-32 rounded-full theme-bg-card border-4 border-[var(--c-bg-card)] flex items-center justify-center shadow-lg overflow-hidden shrink-0 relative">{(selectedShop.package === 'Premium' || selectedShop.package === 'Premium Paket') && <div className="absolute inset-0 border-4 border-yellow-400 rounded-full z-10 pointer-events-none shadow-[0_0_20px_rgba(250,204,21,0.6)]"></div>}{selectedShop.logo_url ? <img alt={`${selectedShop.name} Logosu`} loading="lazy" decoding="async" src={selectedShop.logo_url} className="w-full h-full object-cover" /> : <span className="text-[#E8622A] font-black">LOGO</span>}</div></div></div>
                <div className="px-2 md:px-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b theme-border-sub pb-8 gap-4"><div><div className="flex items-center gap-3 mb-2"><h1 className="text-3xl md:text-5xl font-black uppercase theme-text-main tracking-tight">{selectedShop.name}</h1>{(selectedShop.package === 'Premium' || selectedShop.package === 'Premium Paket') && <Gem size={24} className="text-yellow-500 fill-yellow-500 drop-shadow-md"/>}</div><div className="flex flex-wrap items-center gap-3 text-xs font-bold theme-text-muted uppercase tracking-widest"><span className="flex items-center text-[#E8622A]"><Briefcase size={14} className="mr-1"/> {t[lang].cats[categories.find(c => c.dbName === selectedShop.category)?.key || 'barber']}</span><span className="flex items-center"><MapPin size={14} className="mr-1"/> {selectedShop.maps_link ? (<a href={selectedShop.maps_link.startsWith('http') ? selectedShop.maps_link : `https://${selectedShop.maps_link}`} target="_blank" rel="noopener noreferrer" className="theme-text-muted hover:text-[#E8622A] underline decoration-slate-400 underline-offset-4">{selectedShop.address || selectedShop.location}</a>) : (selectedShop.address || selectedShop.location)}</span></div></div></div>
                    <div className="flex gap-8 border-b theme-border-sub mb-8 overflow-x-auto custom-scrollbar">
                        <button onClick={() => setProfileTab(selectedShop.category === 'Bar & Club' ? 'events' : 'services')} className={`pb-4 text-sm font-black uppercase tracking-widest whitespace-nowrap transition-colors border-b-4 bg-transparent outline-none cursor-pointer ${(profileTab === 'services' || profileTab === 'events') ? 'border-[#E8622A] text-[#E8622A]' : 'border-transparent text-slate-400 hover:theme-text-main'}`}>{selectedShop.category === 'Bar & Club' ? t[lang].profile.tabEvents : t[lang].profile.tabServices}</button>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        <div className="lg:col-span-7">
                                ...
                                <div className="animate-in fade-in flex flex-col gap-4">
                                    {bookingPhase === 1 ? (
                                        selectedShop.events && selectedShop.events.length > 0 ? (
                                            selectedShop.events.map(ev => (<div key={ev.id} onClick={() => { setBookingData({...bookingData, selectedEvent: ev, date: ev.date, time: ev.time}); setBookingPhase(2); }} className="flex flex-col sm:flex-row gap-4 p-4 theme-bg-card rounded-[24px] border-2 theme-border-sub hover:border-[#E8622A] cursor-pointer transition-all hover:shadow-md items-center"><div className="w-full sm:w-32 h-40 sm:h-32 rounded-xl theme-bg-sub overflow-hidden shrink-0 border theme-border flex items-center justify-center">{ev.image_url ? <img alt={`${ev.name} Kapak`} loading="lazy" decoding="async" src={ev.image_url} className="w-full h-full object-cover"/> : <Music size={32} className="text-slate-400"/>}</div><div className="flex-1 flex flex-col justify-center text-center sm:text-left w-full"><h4 className="font-black text-xl theme-text-main uppercase mb-2 leading-tight">{ev.name}</h4><div className="flex items-center justify-center sm:justify-start gap-2 text-[#E8622A] font-bold text-sm mb-2"><Calendar size={16}/> {ev.date} • {ev.time}</div>{ev.description && <p className="text-xs theme-text-muted font-medium line-clamp-2">{ev.description}</p>}</div><div className="w-full sm:w-auto mt-2 sm:mt-0"><button className="w-full sm:w-auto px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all border-none outline-none theme-bg-sub theme-text-muted hover:bg-[#E8622A] hover:text-white cursor-pointer pointer-events-none">SEÇ</button></div></div>))
                                        ) : (<div className="theme-bg-sub theme-border border p-10 rounded-[24px] text-center"><Music size={40} className="mx-auto text-slate-400 mb-4"/><p className="theme-text-muted font-bold tracking-widest uppercase text-xs">Yaklaşan etkinlik bulunmuyor.</p></div>)
                                    ) : (
                                        <><button onClick={() => {setBookingData({...bookingData, selectedEvent: null, selectedShopService: null}); setBookingPhase(1);}} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-[#E8622A] bg-transparent border-none outline-none cursor-pointer flex items-center gap-1 w-fit mb-2"><ChevronLeft size={14}/> Etkinliklere Dön</button>
                                        {selectedShop.services && selectedShop.services.length > 0 ? (
                                            selectedShop.services.map(srv => {
                                                ...
                                                return (<div key={srv.id} onClick={() => { if(!isSoldOut) { setBookingData({...bookingData, selectedShopService: srv}); setBookingPhase(3); } }} className={`p-6 theme-bg-card rounded-[24px] border-2 flex justify-between items-center transition-all ${isSoldOut ? 'theme-border-sub opacity-60 cursor-not-allowed' : bookingData.selectedShopService?.id === srv.id ? 'border-[#E8622A] shadow-md scale-[1.02] cursor-pointer' : 'theme-border-sub hover:border-[#E8622A] cursor-pointer'}`}><div><h4 className="font-black text-lg theme-text-main mb-1">{srv.name}</h4><div className="flex items-center gap-1 text-slate-400"><Music size={12}/><span className="text-[10px] font-bold uppercase tracking-widest">Gece Boyu</span></div></div><div className="flex items-center gap-6"><span className="font-black text-xl theme-text-main">{priceDisplay}</span><button disabled={isSoldOut} className={`px-6 py-2.5 rounded-full font-black text-xs uppercase tracking-widest transition-all border-none outline-none ${isSoldOut ? 'bg-red-500/20 text-red-500' : bookingData.selectedShopService?.id === srv.id ? 'bg-[#E8622A] text-white cursor-pointer' : 'theme-bg-sub theme-text-muted hover:bg-[var(--c-border)] cursor-pointer'}`}>{isSoldOut ? 'TÜKENDİ' : 'SEÇ'}</button></div></div>)
                                            })
                                        ) : (<div className="theme-bg-sub theme-border border p-10 rounded-[24px] text-center"><Ticket size={40} className="mx-auto text-slate-400 mb-4"/><p className="theme-text-muted font-bold tracking-widest uppercase text-xs">Loca listesi bulunamadı.</p></div>)}
                                        </>
                                    )}
                                </div>
                            )}
                            {(profileTab === 'services' || profileTab === 'events') && selectedShop.category !== 'Bar & Club' && (
                                <div className="animate-in fade-in flex flex-col gap-4">
                                    {selectedShop.services && selectedShop.services.length > 0 ? (
                                        selectedShop.services.map(srv => (<div key={srv.id} onClick={() => { setBookingData({...bookingData, selectedShopService: srv, selectedStaff: null, time: ''}); setBookingPhase(2); }} className={`p-6 theme-bg-card rounded-[24px] border-2 flex justify-between items-center cursor-pointer transition-all ${bookingData.selectedShopService?.id === srv.id ? 'border-[#E8622A] shadow-md scale-[1.02]' : 'theme-border-sub hover:border-[#E8622A]'}`}><div><h4 className="font-black text-lg theme-text-main mb-1">{srv.name}</h4><div className="flex items-center gap-1 text-slate-400"><Clock size={12}/><span className="text-[10px] font-bold uppercase tracking-widest">{srv.duration} Dakika</span></div></div><div className="flex items-center gap-6"><span className="font-black text-xl theme-text-main">{!srv.price || srv.price === '0' ? 'ÜCRETSİZ' : `${srv.price} TL`}</span><button className={`px-6 py-2.5 rounded-full font-black text-xs uppercase tracking-widest transition-all border-none outline-none cursor-pointer ${bookingData.selectedShopService?.id === srv.id ? 'bg-[#E8622A] text-white' : 'theme-bg-sub theme-text-muted hover:bg-[var(--c-border)]'}`}>{t[lang].profile.bookBtn}</button></div></div>))
                                    ) : (<div className="theme-bg-sub theme-border border p-10 rounded-[24px] text-center"><Scissors size={40} className="mx-auto text-slate-400 mb-4"/><p className="theme-text-muted font-bold tracking-widest uppercase text-xs">{t[lang].profile.noServices}</p></div>)}
                                </div>
                            )}
                            {profileTab === 'gallery' && (
                                <div className="animate-in fade-in">
                                    {selectedShop.gallery && selectedShop.gallery.length > 0 ? (
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">{selectedShop.gallery.map((imgUrl, idx) => (<div key={idx} onClick={() => setLightboxImg(imgUrl)} className="w-full h-40 md:h-48 rounded-2xl overflow-hidden cursor-pointer group relative theme-border border"><img alt={`Galeri Görseli ${idx+1}`} loading="lazy" decoding="async" src={imgUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /><div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Search className="text-white opacity-0 group-hover:opacity-100 transition-opacity" size={24}/></div></div>))}</div>
                                    ) : (<div className="theme-bg-sub theme-border border p-10 rounded-[24px] text-center"><Grid size={40} className="mx-auto text-slate-400 mb-4"/><p className="theme-text-muted font-bold tracking-widest uppercase text-xs">{t[lang].profile.noGallery}</p></div>)}
                                </div>
                            )}
                            {profileTab === 'about' && (
                                <div className="animate-in fade-in">
                                    ...
                                    <div className="theme-bg-card theme-border border p-8 rounded-[24px] shadow-sm"><h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 mb-4">{t[lang].profile.about}</h3><p className="theme-text-muted text-sm leading-relaxed font-medium whitespace-pre-wrap">{selectedShop.description || <span className="italic opacity-50">{t[lang].profile.noDesc}</span>}</p></div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="lg:col-span-5 relative">
                            <div className="sticky top-28 theme-bg-card theme-border border-2 rounded-[32px] p-6 md:p-8 shadow-[0_20px_40px_rgba(0,0,0,0.06)] flex flex-col min-h-[450px]">
                                <div className="flex justify-between items-center mb-6 border-b theme-border-sub pb-4">
                                    ...
                                    {bookingPhase > 1 && (<button onClick={() => { if (selectedShop.category === 'Bar & Club' && bookingPhase === 2) { setBookingPhase(1); setBookingData({...bookingData, selectedEvent: null}); } else { setBookingPhase(bookingPhase - 1); } }} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-[#E8622A] theme-bg-sub px-3 py-1.5 rounded-lg flex items-center transition-colors border-none outline-none cursor-pointer"><ChevronLeft size={14} className="mr-1"/> {t[lang].book.change}</button>)}
                                </div>
                                {bookingPhase === 1 && (<div className="flex-1 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">{selectedShop.category === 'Bar & Club' ? <><Music size={40} className="text-slate-400 mb-4" /><p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-relaxed max-w-[200px]">{t[lang].book.selectEvent}</p></> : <><Scissors size={40} className="text-slate-400 mb-4" /><p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-relaxed max-w-[200px]">{t[lang].book.selectService}</p></>}</div>)}
                                {bookingPhase === 2 && selectedShop.category === 'Bar & Club' && (<div className="flex-1 flex flex-col animate-in fade-in duration-300"><div className="mb-6 theme-bg-sub p-4 rounded-2xl border theme-border-sub flex flex-col gap-2"><div className="flex justify-between items-center"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.event}</span><span className="font-black theme-text-main text-sm text-right">{bookingData.selectedEvent?.name}</span></div><div className="flex justify-between items-center border-t theme-border-sub pt-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.dateTime}</span><span className="font-bold text-[#E8622A] text-xs text-right">{bookingData.selectedEvent?.date} | {bookingData.selectedEvent?.time}</span></div></div><div className="flex-1 flex flex-col items-center justify-center text-center"><Ticket size={40} className="text-slate-400 mb-4" /><p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-relaxed max-w-[200px]">{t[lang].book.selectLoca}</p></div></div>)}
                                {bookingPhase > 1 && (selectedShop.category !== 'Bar & Club' || bookingPhase > 2) && (
                                    <div className="mb-6 theme-bg-sub p-4 rounded-2xl border theme-border-sub flex flex-col gap-2 animate-in fade-in">
                                        {selectedShop.category === 'Bar & Club' && (<><div className="flex justify-between items-center"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.event}</span><span className="font-black theme-text-main text-sm text-right">{bookingData.selectedEvent?.name}</span></div><div className="flex justify-between items-center border-t theme-border-sub pt-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.dateTime}</span><span className="font-bold text-[#E8622A] text-xs text-right">{bookingData.selectedEvent?.date} | {bookingData.selectedEvent?.time}</span></div></>)}
                                        <div className={`flex justify-between items-center ${selectedShop.category === 'Bar & Club' ? 'border-t theme-border-sub pt-2' : ''}`}><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.service}</span><span className="font-black theme-text-main text-sm text-right">{bookingData.selectedShopService?.name}</span></div>
                                        {bookingPhase > 2 && bookingData.selectedStaff && selectedShop.category !== 'Bar & Club' && (<div className="flex justify-between items-center border-t theme-border-sub pt-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.staff}</span><span className="font-bold theme-text-main text-xs uppercase text-right">{bookingData.selectedStaff.name}</span></div>)}
                                        {bookingPhase > 3 && bookingData.time && selectedShop.category !== 'Bar & Club' && (<div className="flex justify-between items-center border-t theme-border-sub pt-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t[lang].book.dateTime}</span><span className="font-bold text-[#E8622A] text-xs text-right">{bookingData.date} | {bookingData.time}</span></div>)}
                                        <div className="flex justify-between items-center border-t theme-border-sub pt-2 mt-1"><span className="text-[10px] font-black theme-text-main uppercase tracking-widest">{t[lang].book.total}</span><span className="font-black text-[#E8622A] text-lg text-right">{(!bookingData.selectedShopService?.price || bookingData.selectedShopService?.price === '0') ? 'ÜCRETSİZ' : `${bookingData.selectedShopService?.price} TL`}</span></div>
                                    </div>
                                )}
                                {bookingPhase === 2 && selectedShop.category !== 'Bar & Club' && (
                                    <div className="animate-in slide-in-from-right-4 fade-in duration-300 flex-1">
                                        ...
                                        <div className="grid grid-cols-3 gap-3">
                                            <button onClick={() => { setBookingData({...bookingData, selectedStaff: { name: t[lang].book.anyStaff }}); setBookingPhase(3); }} className="flex flex-col items-center gap-2 cursor-pointer p-3 rounded-2xl border-2 border-transparent hover:border-[#E8622A] theme-bg-card shadow-sm hover:shadow-md transition-all bg-transparent"><div className="w-12 h-12 rounded-full flex items-center justify-center theme-bg-sub text-slate-400"><Users size={20}/></div><span className="text-[9px] font-black text-center theme-text-muted uppercase">{t[lang].book.anyStaff}</span></button>
                                            {selectedShop.staff?.map(person => (<button key={person.id} onClick={() => { setBookingData({...bookingData, selectedStaff: person}); setBookingPhase(3); }} className="flex flex-col items-center gap-2 cursor-pointer p-3 rounded-2xl border-2 border-transparent hover:border-[#E8622A] theme-bg-card shadow-sm hover:shadow-md transition-all bg-transparent"><div className="w-12 h-12 rounded-full flex items-center justify-center theme-bg-sub text-slate-400"><UserCircle size={24}/></div><span className="text-[9px] font-black text-center theme-text-muted uppercase truncate w-full px-1">{person.name}</span></button>))}
                                        </div>
                                    </div>
                                )}
                                {bookingPhase === 3 && selectedShop.category !== 'Bar & Club' && (() => {
                                    const currentAvailableSlots = getCurrentAvailableSlots(); const isShopClosedToday = currentAvailableSlots.length === 0;
                                    return (
                                        <div className="animate-in slide-in-from-right-4 fade-in duration-300 flex-1 flex flex-col gap-4">
                                            <div className="flex items-center gap-2 theme-text-main font-black text-[11px] uppercase tracking-widest"><Calendar size={14} className="text-[#E8622A]"/> {t[lang].book.date} / {t[lang].book.time}</div>
                                            <input aria-label="Randevu Tarihi" type="date" min={new Date().toISOString().split('T')[0]} value={bookingData.date} className="w-full theme-bg-sub theme-border rounded-xl py-4 px-4 font-bold theme-text-main outline-none focus:border-[#E8622A] text-sm cursor-pointer transition-colors shadow-inner" onChange={(e) => setBookingData({...bookingData, date: e.target.value, time: ''})} />
                                            {bookingData.date && (
                                                isShopClosedToday ? (
                                                    <div className="py-10 text-center text-red-500 font-bold uppercase tracking-widest text-xs bg-red-500/10 rounded-2xl border border-red-500/20 mt-2"><CalendarOff size={32} className="mx-auto mb-2 opacity-50"/>{t[lang].book.shopClosed}</div>
                                                ) : (
                                                    <div className="grid grid-cols-3 gap-2 max-h-[180px] overflow-y-auto pr-2 custom-scrollbar mt-2">
                                                        {currentAvailableSlots.map((slot, idx) => {
                                                            const neededSlots = getRequiredSlots(bookingData.selectedShopService.duration); const slotsToCheck = currentAvailableSlots.slice(idx, idx + neededSlots); let isUnavailable = slotsToCheck.length < neededSlots; 
                                                            if (!isUnavailable) {
                                                                if (bookingData.selectedStaff?.name === t[lang].book.anyStaff || bookingData.selectedStaff?.name === 'Fark Etmez') {
                                                                    if (selectedShop.staff && selectedShop.staff.length > 0) { isUnavailable = !selectedShop.staff.some(staff => slotsToCheck.every(checkSlot => { if (closedSlots.includes(checkSlot)) return false; return !appointments.some(a => a.staff_name === staff.name && (a.occupied_slots ? a.occupied_slots.includes(checkSlot) : a.appointment_time === checkSlot)); })); } else { isUnavailable = slotsToCheck.some(checkSlot => closedSlots.includes(checkSlot) || appointments.some(a => a.occupied_slots ? a.occupied_slots.includes(checkSlot) : a.appointment_time === checkSlot)); }
                                                                } else { isUnavailable = slotsToCheck.some(checkSlot => closedSlots.includes(checkSlot) || appointments.some(a => a.staff_name === bookingData.selectedStaff.name && (a.occupied_slots ? a.occupied_slots.includes(checkSlot) : a.appointment_time === checkSlot))); }
                                                            }
                                                            return (
                                                                <button key={slot} disabled={isUnavailable} onClick={() => { setBookingData({...bookingData, time: slot}); setBookingPhase(4); }} className={`p-3 rounded-xl text-xs font-bold border-2 transition-all cursor-pointer ${isUnavailable ? 'theme-bg-sub border-transparent text-slate-400 cursor-not-allowed' : bookingData.time === slot ? 'bg-[#E8622A] text-white border-[#E8622A] shadow-md' : 'theme-bg-card theme-border theme-text-muted hover:border-[#E8622A] hover:text-[#E8622A]'}`}>{slot}</button>
                                                            );
                                                        })}
                                                    </div>
                                                )
                                ...
                                {(bookingPhase === 4 || (selectedShop.category === 'Bar & Club' && bookingPhase === 3)) && (
                                    <form onSubmit={handleBooking} className="animate-in slide-in-from-right-4 fade-in duration-300 flex flex-col gap-3 flex-1 mt-auto">
                                        <div className="flex items-center gap-2 theme-text-main font-black text-[11px] uppercase tracking-widest mb-1 border-t theme-border-sub pt-4"><User size={14} className="text-[#E8622A]"/> {t[lang].book.contactInfo}</div>
                                        <div className="flex gap-2 w-full"><input aria-label="Adınız" required placeholder={t[lang].book.name} className="w-full theme-bg-sub theme-border rounded-[14px] py-4 px-4 outline-none focus:border-[#E8622A] font-bold text-xs theme-text-main transition-colors" onChange={(e) => setFormData({...formData, name: e.target.value})} /><input aria-label="Soyadınız" required placeholder={t[lang].book.surname} className="w-full theme-bg-sub theme-border rounded-[14px] py-4 px-4 outline-none focus:border-[#E8622A] font-bold text-xs theme-text-main transition-colors" onChange={(e) => setFormData({...formData, surname: e.target.value})} /></div>
                                        <div className="flex gap-2 w-full relative">
                                          <select aria-label="Telefon Kodu" className="theme-bg-sub theme-border rounded-[14px] py-4 px-3 outline-none focus:border-[#E8622A] font-bold text-xs theme-text-main cursor-pointer w-28 shrink-0" value={formData.phoneCode} onChange={e => setFormData({...formData, phoneCode: e.target.value})}><option value="+90">TR (+90)</option><option value="+357">CY (+357)</option><option value="+44">UK (+44)</option></select>
                                          <div className="relative flex-1"><input aria-label="Telefon Numarası" required type="tel" placeholder={t[lang].book.phone} className="w-full theme-bg-sub theme-border rounded-[14px] py-4 px-4 pr-10 outline-none focus:border-[#E8622A] font-bold text-xs theme-text-main transition-colors" onChange={handleBookingPhoneChange} />{bookingPhoneValid === true && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500" size={16}/>}{bookingPhoneValid === false && <XCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500" size={16}/>}</div>
                                        </div>
                                        <div className="relative w-full"><input aria-label="E-Posta Adresi" required type="email" placeholder={t[lang].book.email} className="w-full theme-bg-sub theme-border rounded-[14px] py-4 px-4 pr-10 outline-none focus:border-[#E8622A] font-bold text-xs theme-text-main transition-colors" onChange={handleBookingEmailChange} />{bookingEmailValid === true && <CheckCircle2 className="absolute right-3 top-1/2 -translate-y-1/2 text-green-500" size={16}/>}{bookingEmailValid === false && <XCircle className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500" size={16}/>}</div>
                                        <button type="submit" disabled={bookingEmailValid === false || bookingPhoneValid === false} className="w-full bg-[#E8622A] text-white py-5 rounded-[14px] mt-2 uppercase font-black text-xs tracking-widest shadow-xl shadow-[#E8622A]/20 border-none cursor-pointer hover:bg-orange-500 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">{t[lang].book.btnBook}</button>
                                    </form>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="featured-grid">
                                {similarShops.map((shop) => (
                                    <div key={shop.id} onClick={() => { setSelectedShop(shop); setStep('shop_profile'); setProfileTab(shop.category === 'Bar & Club' ? 'events' : 'services'); setBookingData({date: new Date().toISOString().split('T')[0], time:'', selectedShopService: null, selectedStaff: null, selectedEvent: null}); setBookingPhase(1); window.scrollTo(0,0); }} className="venue-card flex flex-col cursor-pointer theme-bg-card theme-border hover:border-[#E8622A] transition-colors">
                                        <div className="venue-img" style={{background: 'var(--c-bg-sub)'}}>{shop.cover_url || shop.logo_url ? <img alt={`${shop.name} Logosu`} loading="lazy" decoding="async" src={shop.cover_url || shop.logo_url} /> : categories.find(c=>c.dbName===shop.category)?.emoji}{(shop.package === 'Premium' || shop.package === 'Premium Paket') && <div className="venue-badge hot">🔥 VIP</div>}<div className="venue-fav"><HeartHandshake size={14}/></div></div>
                                        <div className="venue-info p-5"><div className="venue-cat">{t[lang].cats[categories.find(c => c.dbName === shop.category)?.key || 'barber']}</div><div className="venue-name">{shop.name}</div><div className="venue-meta"><span>📍 {shop.location}</span></div></div>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <div className="w-full bg-[#FAF7F2] animate-in fade-in overflow-hidden pb-24">
                ...
                <div className="bg-white py-20 border-y border-slate-200">
                    <div className="max-w-[1200px] mx-auto px-4 md:px-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            <div className="bg-slate-50 p-8 rounded-[24px] border border-slate-200"><div className="w-12 h-12 bg-white text-[#E8622A] rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100"><Crown size={24}/></div><h3 className="font-black text-lg text-[#2D1B4E] mb-3">Öncü Platform</h3><p className="text-sm text-slate-500 font-medium">Kıbrıs’ta ilk ve öncü randevu platformlarından biri.</p></div>
                            <div className="bg-slate-50 p-8 rounded-[24px] border border-slate-200"><div className="w-12 h-12 bg-white text-[#E8622A] rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100"><Grid size={24}/></div><h3 className="font-black text-lg text-[#2D1B4E] mb-3">Entegre Sistem</h3><p className="text-sm text-slate-500 font-medium">Farklı sektörleri kapsayan tek entegre sistem.</p></div>
                            <div className="bg-slate-50 p-8 rounded-[24px] border border-slate-200"><div className="w-12 h-12 bg-white text-[#E8622A] rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100"><Users size={24}/></div><h3 className="font-black text-lg text-[#2D1B4E] mb-3">Gerçek Müşteri</h3><p className="text-sm text-slate-500 font-medium">İşletmelere gerçek müşteri kazandıran aktif trafik.</p></div>
                            <div className="bg-slate-50 p-8 rounded-[24px] border border-slate-200"><div className="w-12 h-12 bg-white text-[#E8622A] rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100"><Smartphone size={24}/></div><h3 className="font-black text-lg text-[#2D1B4E] mb-3">Üst Düzey Arayüz</h3><p className="text-sm text-slate-500 font-medium">Sade, hızlı ve kullanıcı dostu modern tasarım.</p></div>
                            <div className="bg-slate-50 p-8 rounded-[24px] border border-slate-200"><div className="w-12 h-12 bg-white text-[#E8622A] rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100"><TrendingUp size={24}/></div><h3 className="font-black text-lg text-[#2D1B4E] mb-3">Maksimum Kazanç</h3><p className="text-sm text-slate-500 font-medium">Komisyonsuz model ile gelirinizi katlayın.</p></div>
                            <div className="bg-slate-50 p-8 rounded-[24px] border border-slate-200"><div className="w-12 h-12 bg-white text-[#E8622A] rounded-xl flex items-center justify-center mb-6 shadow-sm border border-slate-100"><MessageSquare size={24}/></div><h3 className="font-black text-lg text-[#2D1B4E] mb-3">Gelişmiş Otomasyon</h3><p className="text-sm text-slate-500 font-medium">Yakında eklenecek SMS ve gelişmiş bildirim sistemleri.</p></div>
                        </div>
                        <div className="text-center mt-12 font-black text-[#2D1B4E] text-lg bg-orange-50 py-4 rounded-xl border border-orange-100">👉 Bookcy, sadece bir platform değil, işletmenizin büyüme motorudur.</div>
                    </div>
                </div>
