  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .nav-links button { text-decoration: none; font-size: 14px; font-weight: 700; color: var(--fig) !important; opacity: 0.8; transition: opacity 0.2s; position: relative; background:none; border:none; outline:none; font-family:'DM Sans', sans-serif; cursor:pointer;}
        .nav-links button:hover, .nav-links button.active { opacity:1; color: var(--terra) !important; }
        ...
        .btn-outline { font-family:'DM Sans',sans-serif; font-size: 13px; font-weight: 700; padding: 9px 20px; border-radius: 50px; border: 1.5px solid var(--fig); background: transparent; color: var(--fig); transition: all 0.25s; cursor:pointer;}
        .btn-outline:hover { background:var(--fig); color:var(--c-bg-card); }
                              ...
                              <div className="grid grid-cols-4 gap-8 mb-10 text-left">
                                  <div>
                                    ...
                                    <ul className="space-y-5 font-bold text-slate-600 capitalize text-sm">
                                      {['profile', 'market', 'team'].map(key => <li key={key} onClick={() => goToFeature(key)} className="hover:text-[#2D1B4E] cursor-pointer transition-colors flex items-center gap-2"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</li>)}
                                    </ul>
                                  </div>
                                  <div>
                                    <h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col2Title}</h4>
                                    <ul className="space-y-5 font-bold text-slate-600 capitalize text-sm">
                                      {['booking', 'app'].map(key => <li key={key} onClick={() => goToFeature(key)} className="hover:text-[#2D1B4E] cursor-pointer transition-colors flex items-center gap-2"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</li>)}
                                    </ul>
                                  </div>
                                  <div>
                                    <h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col3Title}</h4>
                                    <ul className="space-y-5 font-bold text-slate-600 capitalize text-sm">
                                      {['marketing', 'calendar', 'crm'].map(key => <li key={key} onClick={() => goToFeature(key)} className="hover:text-[#2D1B4E] cursor-pointer transition-colors flex items-center gap-2"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</li>)}
                                    </ul>
                                  </div>
                                  <div>
                                    <h4 className="font-black text-[11px] uppercase tracking-widest mb-6 text-[#E8622A]">{t[lang].megaMenu.col4Title}</h4>
                                    <ul className="space-y-5 font-bold text-slate-600 capitalize text-sm">
                                      {['boost', 'stats'].map(key => <li key={key} onClick={() => goToFeature(key)} className="hover:text-[#2D1B4E] cursor-pointer transition-colors flex items-center gap-2"><ChevronRight size={14} className="text-[#E8622A]"/> {t[lang].featNames[key]}</li>)}
                                    </ul>
                                  </div>
                              </div>
