        ...
        <div className="flex-1 w-full flex flex-col gap-5">
            {sortedShops.map((shop) => (
                <div key={shop.id} onClick={() => router.push(`/isletmeler/${shop.id}`)} className="flex flex-col sm:flex-row items-center p-6 bg-white border border-slate-200 hover:border-[#E8622A] rounded-[32px] cursor-pointer transition-all shadow-sm hover:shadow-md">
                    <div className="w-24 h-24 rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden shrink-0 flex items-center justify-center text-xs font-black text-[#E8622A] relative">
                      {(shop.package === 'Premium Paket' || shop.package === 'Premium') && <div className="absolute inset-0 border-4 border-yellow-400 rounded-2xl"></div>}
                      {shop.logo_url ? <img loading="lazy" src={shop.logo_url} className="w-full h-full object-cover" /> : (categories.find(c=>c.dbName===shop.category)?.emoji || '💈')}
                    </div>
                    <div className="flex-1 sm:ml-6 mt-4 sm:mt-0 text-center sm:text-left">
                      <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
                        ...
                        {(shop.package === 'Premium Paket' || shop.package === 'Premium') && <Gem size={16} className="text-yellow-500 fill-yellow-500"/>}
                      </div>
                      <div className="flex flex-wrap justify-center sm:justify-start gap-3 mt-2 text-slate-500 text-[10px] font-bold uppercase tracking-widest">
                        <span className="bg-slate-100 px-3 py-1.5 rounded-lg">{shop.category}</span>
