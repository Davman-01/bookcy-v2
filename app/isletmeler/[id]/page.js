        ...
        <div className="text-center py-20 px-6 flex flex-col items-center justify-center max-w-[600px] mx-auto animate-in zoom-in-95 duration-500">
          {!feedbackSubmitted ? (
            <div className="bg-white rounded-[40px] p-8 md:p-14 shadow-2xl border border-slate-200 w-full relative overflow-hidden">
              ...
              <h2 className="text-3xl md:text-4xl font-black text-[#2D1B4E] uppercase mb-4 tracking-tight">{text.success.title}</h2>
              <p className="text-slate-500 font-bold text-sm md:text-base mb-10 leading-relaxed">{text.success.desc}</p>
              <div className="border-t border-slate-100 pt-10 mt-6 bg-slate-50 -mx-8 md:-mx-14 -mb-8 md:-mb-14 px-8 md:px-14 pb-8 md:pb-14">
                <h3 className="font-black text-xl text-[#2D1B4E] mb-8 flex items-center justify-center gap-2"><Star className="text-yellow-500 fill-yellow-500"/> {text.success.rateTitle}</h3>
                <form onSubmit={submitFeedback} className="text-left space-y-8">
                  <div><label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-3 block text-center">{text.success.q1}</label>{renderFeedbackScale('q1')}</div>
                  <div><label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-3 block text-center">{text.success.q2}</label>{renderFeedbackScale('q2')}</div>
                  <div><label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-3 block text-center">{text.success.q3}</label>{renderFeedbackScale('q3')}</div>
                  <div><label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-3 block text-center">{text.success.q4}</label>{renderFeedbackScale('q4')}</div>
                  <button type="submit" className="w-full bg-[#2D1B4E] text-white py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs border-none cursor-pointer shadow-xl hover:bg-[#1a0f2e] transition-all hover:-translate-y-1 mt-4">{text.success.btn}</button>
                </form>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-[40px] p-10 md:p-20 shadow-2xl border border-slate-200 w-full text-center"><div className="w-32 h-32 bg-yellow-50 text-yellow-500 rounded-full flex items-center justify-center mx-auto mb-10 border-4 border-yellow-100"><Star fill="currentColor" size={64}/></div><h2 className="text-4xl font-black text-[#2D1B4E] uppercase mb-4 tracking-tight">{text.success.thanks}</h2><p className="text-slate-500 mb-12 font-medium text-lg">{text.success.saved}</p><button onClick={() => router.push('/')} className="bg-[#E8622A] text-white mx-auto px-12 py-5 rounded-full font-black uppercase tracking-[0.2em] text-sm border-none cursor-pointer shadow-xl hover:bg-[#d4561f] transition-all hover:scale-105 w-full md:w-auto">{text.success.home}</button></div>
          )}
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-5 relative mt-10 lg:mt-0">
              <div className="lg:sticky lg:top-28 bg-white border border-slate-200 rounded-[40px] p-6 md:p-10 flex flex-col min-h-[450px] shadow-xl mb-10 lg:mb-0">
                  ...
                  {bookingPhase === 1 && (<div className="flex-1 flex flex-col items-center justify-center text-center p-10">{selectedShop.category === 'Bar & Club' ? <><div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-6"><Music size={40} className="text-slate-300"/></div><p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-relaxed">{text.booking.startEvent}</p></> : <><div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-6"><Scissors size={40} className="text-slate-300"/></div><p className="text-xs font-bold text-slate-500 uppercase tracking-widest leading-relaxed">{text.booking.startSrv}</p></>}</div>)}
                  
                  {bookingPhase > 1 && (
                      <div className="mb-8 bg-slate-50 p-6 rounded-3xl border border-slate-200 flex flex-col gap-4 shadow-inner">
                          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{text.booking.srv}</span><span className="font-black text-[#2D1B4E] text-sm md:text-base">{bookingData.selectedShopService?.name}</span></div>
                          {bookingPhase > 2 && bookingData.selectedStaff && selectedShop.category !== 'Bar & Club' && (<div className="flex flex-col sm:flex-row justify-between sm:items-center border-t border-slate-200 pt-4 gap-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{text.booking.staff}</span><span className="font-bold text-[#2D1B4E] text-xs uppercase bg-white px-3 py-1.5 rounded-lg border border-slate-100">{bookingData.selectedStaff.name}</span></div>)}
                          {bookingPhase > 3 && bookingData.time && selectedShop.category !== 'Bar & Club' && (<div className="flex flex-col sm:flex-row justify-between sm:items-center border-t border-slate-200 pt-4 gap-2"><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{text.booking.time}</span><span className="font-bold text-[#E8622A] text-xs bg-orange-50 px-3 py-1.5 rounded-lg border border-orange-100">{bookingData.date} | {bookingData.time}</span></div>)}
                          <div className="flex flex-col sm:flex-row justify-between sm:items-center border-t border-slate-200 pt-4 mt-2 gap-2"><span className="text-[10px] font-black text-[#2D1B4E] uppercase tracking-widest">{text.booking.total}</span><span className="font-black text-[#E8622A] text-2xl">{bookingData.selectedShopService?.price || '0'} TL</span></div>
                      </div>
                  )}
