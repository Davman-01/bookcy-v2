...
export const getBookingConfirmationTemplate = (data) => {
  const { customerName, shopName, date, time, service, staff, address, customerPhone, customerEmail } = data;
  return `
    <div style="${baseStyle}">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background-color:#ffffff;border-radius:16px;overflow:hidden;">
        <tr><td style="background-color:#4A1942;padding:36px 40px 28px;">
        ...
        <tr><td style="padding:32px 40px 0;text-align:center;">
          ...
          <a href="https://www.bookcy.co/randevu-sorgula?phone=${customerPhone}&email=${customerEmail}" style="display:inline-block;background:#C4622D;color:#ffffff;text-decoration:none;padding:14px 28px;border-radius:50px;font-weight:700;font-size:14px;box-shadow: 0 4px 6px rgba(196, 98, 45, 0.3);">Randevumu Yönet / İptal Et</a>
        </td></tr>
...
// 5. RANDEVU İPTAL BİLDİRİMİ (Müşteri iptal ettiğinde İşletmeye Giden)
export const getBookingCancellationTemplate = (data) => {
  const { shopName, customerName, date, time, service } = data;
  return `
    <div style="${baseStyle}">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background-color:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #fee2e2;">
        <tr><td style="background-color:#ef4444;padding:30px 40px 20px;text-align:center;">
          <div style="font-size:22px;font-weight:900;color:white;text-transform:uppercase;letter-spacing:1px;">DİKKAT: RANDEVU İPTALİ</div>
        </td></tr>
        <tr><td style="padding:40px 40px 30px;">
          <p style="font-size:16px;color:#334155;line-height:1.6;margin:0 0 20px 0;">Sayın <strong>${shopName}</strong>,</p>
          <p style="font-size:15px;color:#475569;line-height:1.6;margin:0 0 24px 0;">Müşteriniz <strong>${customerName}</strong>, aşağıdaki randevusunu sistem üzerinden iptal etmiştir. Bu saat dilimi takviminizde yeniden <strong>müsait</strong> hale gelmiştir.</p>
          
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#fef2f2;border-radius:12px;border-left:4px solid #ef4444;">
            <tr><td style="padding:16px 20px;border-bottom:1px solid #fee2e2;">📅 Tarih: <strong>${date}</strong></td></tr>
            <tr><td style="padding:16px 20px;border-bottom:1px solid #fee2e2;">🕐 Saat: <strong>${time}</strong></td></tr>
            <tr><td style="padding:16px 20px;">✂️ Hizmet: <strong>${service}</strong></td></tr>
          </table>
        </td></tr>
        <tr><td style="padding:20px 40px 30px;text-align:center;">
          <a href="https://www.bookcy.co/isletme-giris" style="display:inline-block;background:#334155;color:white;text-decoration:none;padding:12px 30px;border-radius:8px;font-weight:bold;font-size:14px;">Panele Git</a>
        </td></tr>
      </table>
    </div>
  `;
