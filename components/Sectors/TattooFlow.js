...
export default function TattooFlow({ shop, bookingHelpers, currentAvailableSlots }) {
    ...
    // --- VERİ GARANTİSİ (Sosyal Medya, Telefon) ---
    ...
    const isShopClosedToday = false; // Backend bağlandığında güncellenir
    ...
    return (
        <div className="w-full">
            ...
            <div className="flex gap-6 md:gap-10 border-b border-slate-200 mb-10 overflow-x-auto custom-scrollbar px-2 md:px-6 relative z-10">
                ...
                <button onClick={() => setProfileTab('piercing')} className={`pb-4 text-sm font-black uppercase whitespace-nowrap border-none cursor-pointer border-b-4 transition-colors ${profileTab === 'piercing' ? 'border-[#E8622A] text-[#E8622A]' : 'border-transparent text-slate-400 hover:text-[#2D1B4E]'}`}>PİERCİNG</button>
            ...
            {/* İçerik Düzeni: Piercing ise 2 sütun (Sol Hizmetler, Sağ Randevu), diğerlerinde tek sütun. */}
            <div className={profileTab === 'piercing' ? "grid grid-cols-1 lg:grid-cols-12 gap-10" : "w-full max-w-5xl mx-auto"}>
                
                <div className={profileTab === 'piercing' ? "lg:col-span-7" : "w-full"}>
                    
                    {/* 1. DÖVME (Sadece Form Görünür) */}
                    {profileTab === 'quote' && (
                ...
                {/* SADECE PIERCING: SAĞ PANEL (Randevu Formu) */}
                {profileTab === 'piercing' && (
